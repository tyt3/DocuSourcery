  ...
  try {
    ...
    await Promise.all(
      tagList.map(async (tagId) => {
        ...
        if (!updatedTag) {
          console.log(
            `Project ${projectId} already exists in tag ${tagId} or tag not found.`
          );
        } else {
          console.log(
            `Project ${projectId} appended to tag ${tagId} successfully.`
          );
        }
      })
    );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      project.public =
        publicChoice !== undefined ? publicChoice : project.public;
...
router.post("/project/restore/:id", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    if (!project.deleted) {
      return res
        .status(400)
        .send("Project is not marked as deleted and cannot be restored.");
    }
...
router.get("/project/:slug", async (req, res) => {
  ...
  try {
    ...
    if (
      (project.permissions.loginRequired && req.user) ||
      !project.permissions.loginRequired
    ) {
...
router.get("/pin/:id", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("Server error occurred while trying to pin a project.");
  }
});
...
router.get(
  "/project/:projectSlug/document/create",
  ensureAuth,
  async (req, res) => {
    const projectSlug = req.params.projectSlug;
...
// Edit Document View
router.get(
  "/project/:projectSlug/:documentSlug/edit",
  ensureAuth,
  async (req, res) => {
...
router.post("/document/edit/:id", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    document.landingPage =
      landingPage !== undefined ? landingPage : document.landingPage;
...
router.post("/document/restore/:id", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    if (!document.deleted) {
      ...
      return res
        .status(400)
        .send("Document is already active and not marked as deleted.");
    }
...
router.get("/project/:projectSlug/:documentSlug/", async (req, res) => {
  ...
  try {
    ...
    if (!document) {
      return res
        .status(404)
        .send("Document not found in the specified project");
    }
...
// Edit Page View
router.get(
  "/project/:projectSlug/:documentSlug/:pageSlug/edit",
  ensureAuth,
  async (req, res) => {
    try {
...
// View Page
router.get(
  "/project/:projectSlug/:documentSlug/:pageSlug",
  async (req, res) => {
    const { projectSlug, documentSlug, pageSlug } = req.params;
    try {
      // Get project by its slug and populate users, documents, and pages
...
router.get("/trash", ensureAuth, async (req, res) => {
  try {
    ...
    const trash = await Project.find({
      users: {
        user: req.user._id,
        role: 3,
      },
